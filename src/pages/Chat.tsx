...
// Ensure fetch updates the state:
// useEffect(() => {
...
// ];
  ...
  const handleSendMessage = (content: string) => {
    ...
    setTimeout(async () => {
      try {
        const agentResponseContent = await getAgentResponse(content); // Await the response
        const agentResponse: Message = {
          id: (Date.now() + 1).toString(),
          content: agentResponseContent,
          role: "agent",
        };
        setMessages((prev) => [...prev, agentResponse]); // Add the agent response to messages
      } catch (error) {
        console.error("Error fetching agent response:", error);
        // Optionally, handle the error by adding an error message to the messages state
        setMessages((prev) => [
          ...prev,
          {
            id: (Date.now() + 1).toString(),
            content: "An error occurred while getting the agent's response.",
            role: "agent",
          },
        ]);
      }
